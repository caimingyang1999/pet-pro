...
- [一、接口概述](#一接口概述)
- [二、用户认证模块](#二用户认证模块)
- [三、宠物管理模块](#三宠物管理模块)（含 3.6~3.13 体重记录与成长相册）
- [四、动态模块](#四动态模块)
- [五、商城模块](#五商城模块)
- [六、轮播图模块](#六轮播图模块)
- [七、用户中心模块](#七用户中心模块)
...
**接口说明**：为指定宠物添加一条体重记录。仅可操作自己的宠物。

| 项 | 值 |
|----|----|
| 接口路径 | `POST /pet/weight` |
| 是否登录 | 是 |
| 权限标识 | - |
...
|--------|------|------|------|
| petId | long | 是 | 宠物ID |
| weight | decimal | 是 | 体重(kg)，0~200，保留 2 位小数 |
| recordDate | string | 否 | 记录日期，格式 `yyyy-MM-dd`，不传取当天 |
| remark | string | 否 | 备注，最长 200 字 |

**请求示例（cURL）**：

```bash
curl -X POST 'http://localhost:8080/api/v1/pet/weight' \
  -H 'Authorization: Bearer eyJhbGciOiJIUzUxMiJ9...' \
  -H 'Content-Type: application/json' \
  -d '{"petId":1,"weight":4.5,"recordDate":"2026-09-10","remark":"空腹称重"}'
```
...
**接口说明**：查询指定宠物的体重记录，按记录日期升序返回，便于前端绘制趋势曲线。

| 项 | 值 |
|----|----|
| 接口路径 | `GET /pet/{petId}/weight` |
| 是否登录 | 是 |
| 权限标识 | - |
...
```bash
curl -X GET 'http://localhost:8080/api/v1/pet/1/weight' \
  -H 'Authorization: Bearer eyJhbGciOiJIUzUxMiJ9...'
```
...
```json
{
  ...
  "msg": "操作成功",
  "data": [
    { "id": 10, "petId": 1, "userId": 3, "weight": 4.2, "recordDate": "2026-08-01", "remark": "", "createTime": "2026-08-01 09:12:33" },
    { "id": 12, "petId": 1, "userId": 3, "weight": 4.5, "recordDate": "2026-09-10", "remark": "空腹称重", "createTime": "2026-09-10 08:03:11" }
  ]
}
```
...
**接口说明**：逻辑删除一条体重记录（校验归属）。

| 项 | 值 |
|----|----|
| 接口路径 | `DELETE /pet/weight/{id}` |
| 是否登录 | 是 |
| 权限标识 | - |
...
**接口说明**：为指定宠物添加一条成长相册记录。**内容仅本人可见**，不对外公开，也不提供分享能力。
...
| content | string | 否 | 描述，最长 500 字 |
| images | string | 否 | 图片列表，JSON 数组字符串，元素为上传接口返回的相对路径 |
| recordDate | string | 否 | 记录日期，格式 `yyyy-MM-dd`，不传取当天 |

**请求示例（cURL）**：

```bash
curl -X POST 'http://localhost:8080/api/v1/pet/album' \
  -H 'Authorization: Bearer eyJhbGciOiJIUzUxMiJ9...' \
  -H 'Content-Type: application/json' \
  -d '{"petId":1,"title":"第一次学会握手","content":"花了三天就学会了","images":"[\"/profile/upload/2026/09/10/a.jpg\"]","recordDate":"2026-09-10"}'
```
...
**接口说明**：查询指定宠物的成长相册记录，按记录日期倒序返回。

| 项 | 值 |
|----|----|
| 接口路径 | `GET /pet/{petId}/album` |
| 是否登录 | 是 |
| 权限标识 | - |
  ...
  "data": [
    {
      ...
      "title": "第一次学会握手",
      "content": "花了三天就学会了",
      "images": "["/profile/upload/2026/09/10/a.jpg"]",
      "recordDate": "2026-09-10",
      "createTime": "2026-09-10 20:11:02"
    }
  ]
}
```
...
**接口说明**：查询一条成长相册记录详情。**非本人记录一律返回无权限**，避免泄露他人内容。

| 项 | 值 |
|----|----|
| 接口路径 | `GET /pet/album/{id}` |
| 是否登录 | 是 |
| 权限标识 | - |
...
**接口说明**：更新一条成长相册记录（校验归属，归属与所属宠物不可被篡改）。

| 项 | 值 |
|----|----|
| 接口路径 | `PUT /pet/album/{id}` |
| 是否登录 | 是 |
| 权限标识 | - |
...
**请求体（Body）**：字段同 3.9（`petId` 会被忽略，以原记录为准）。
...
**接口说明**：逻辑删除一条成长相册记录（校验归属）。

| 项 | 值 |
|----|----|
| 接口路径 | `DELETE /pet/album/{id}` |
| 是否登录 | 是 |
| 权限标识 | - |
...
> 模块路径：`/api/v1/posts`
>
> **⚠️ 合规说明（2026-09-10）**：为通过微信个人主体小程序审核，小程序已由「社交-笔记」类目
> 转型为「宠物健康管理工具」，前端**已彻底移除**发布动态、评论、点赞、关注、用户主页等全部
> 社交入口与页面，本章接口前端不再调用。
> 后端代码与 `pet_post` / `post_comment` / `post_like` / `user_follow` 数据表**均保留**，
> 后续若升级为企业主体可直接复用，无需重写。
...
> 模块路径：`/api/v1/articles`
>
> 内容由平台（管理后台）发布，小程序端**只读浏览**，不提供评论、点赞、分享等任何互动能力。
> 两个接口均标注 `@Anonymous`，**游客无需登录即可浏览**（合规要求：浏览不受阻）。

---

### 11.1 养宠知识列表

**接口说明**：分页查询已发布的养宠知识文章，支持按分类与关键词筛选。仅返回 `status = 1`（已发布）的文章，草稿不对外可见。

| 项 | 值 |
|----|----|
| 接口路径 | `GET /articles/list` |
| 是否登录 | 否（`@Anonymous`） |
| 权限标识 | - |
...
| pageNum | int | 否 | 1 | 当前页码 |
| pageSize | int | 否 | 10 | 每页条数，最大100 |
| category | string | 否 | - | 分类（如：喂养 / 健康 / 训练 / 洗护） |
| keyword | string | 否 | - | 关键词，模糊匹配标题或摘要 |

**请求示例（cURL）**：

```bash
curl -X GET 'http://localhost:8080/api/v1/articles/list?pageNum=1&pageSize=5&category=健康'
```
  ...
  "rows": [
    {
      "id": 1,
      "title": "幼犬疫苗接种全流程指南",
      "summary": "从 6 周龄开始，幼犬需要按程序完成三针联苗与一针狂犬疫苗。",
      "coverImage": "/profile/upload/2026/09/10/cover1.jpg",
      "category": "健康",
      "tags": "疫苗,幼犬,免疫",
      "source": "宠迹编辑部",
      "viewCount": 128,
...
> 列表接口**不返回 `content` 正文**字段（正文体积大，仅在详情接口返回）。

---

### 11.2 养宠知识详情

**接口说明**：查询已发布文章详情，并自动将浏览量 +1。未发布或已删除的文章返回业务异常。
...
> **前端渲染说明**：`content` 为富文本 HTML，小程序端使用 `<rich-text :nodes="content" />` 渲染。
> 若正文中图片使用相对路径（`/profile/...`），前端需自行补全域名后再传入。
> 详情页**无评论区、无点赞、无分享入口**。
...
> 模块路径：`/api/v1/admin/articles`
>
> 供 PC 管理后台使用，类级权限标识 `article:list`（增删改分别为 `article:add` / `article:edit` / `article:remove`）。
> 超管（`sys_user.user_id = 1`，拥有 `*:*:*`）可直接访问。
...
| status | string | 否 | 状态（0-草稿 1-已发布） |
| pageNum / pageSize | int | 否 | 分页参数，由 `startPage()` 处理 |
...
```json
{
  ...
  "total": 5,
  "rows": [
    { "id": 1, "title": "幼犬疫苗接种全流程指南", "category": "健康", "status": "1", "viewCount": 128, "sortOrder": 1, "createTime": "2026-09-10 10:00:00" }
  ]
}
```
...
| 项 | 值 |
|----|----|
| 接口路径 | `GET /admin/articles/{id}` |
| 是否登录 | 是 |
| 权限标识 | `article:list` |

**路径参数**：`id` (long, 必填) — 文章ID
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| title | string | 是 | 文章标题，最长 100 字 |
| summary | string | 否 | 摘要，最长 255 字 |
| coverImage | string | 否 | 封面图地址 |
| content | string | 否 | 正文富文本 HTML |
| category | string | 否 | 分类 |
| tags | string | 否 | 标签，逗号分隔 |
| source | string | 否 | 来源，默认「宠迹编辑部」 |
| sortOrder | int | 否 | 排序号，数字越小越靠前，默认 0 |
| status | string | 否 | 状态（0-草稿 1-已发布），默认 `1` |

> `createBy` 由后端自动写入当前登录用户名；`viewCount` 初始化为 0。
...
> ⚠️ **`viewCount` 不允许通过本接口覆盖**，后端会强制置空，浏览量只能由用户阅读行为累加。
...
**路径参数**：`id` (long, 必填) — 逻辑删除，不物理删除数据。

> **备注**：管理后台对应的前端页面（`ruoyi-ui`）尚未开发，如需在后台维护文章，
> 需在 `sys_menu` 中新增「养宠知识管理」菜单（权限标识 `article:list`）并开发对应页面；
> 当前可直接通过本接口或数据库维护文章数据。

---

## 十三、公共数据结构

### 13.1 通用响应对象（AjaxResult）
...
| 动态审核状态（已下架） | status | 0/1/2 | 0-待审核 1-通过 2-拒绝；前端已无动态入口 |
| 商品状态 | status | 0/1 | 0-下架 1-上架 |
| 分类状态 | status | 0/1 | 0-停用 1-正常 |
| 轮播图状态 | status | 0/1 | 0-停用 1-启用 |
| 订单状态 | status | 0/1/2/3 | 0-待发货 1-已发货 2-已完成 3-已取消 |
| 轮播图跳转类型 | jumpType | string | none-不跳转 **article-养宠知识详情** product-商品详情 url-外部链接 miniapp-小程序页面（`post` 已废弃，前端不再处理） |
| 积分变动类型 | changeType | string | sign_in-签到 pet_complete-完善宠物 exchange-兑换商品 admin-管理员操作；post-内容贡献 / like-点赞 / comment-评论 为历史遗留类型，前端已无对应入口 |
| 默认地址 | isDefault | 0/1 | 0-否 1-是 |
| 文章状态 | status | 0/1 | 0-草稿 1-已发布 |
| 登录类型 | loginType | string | wx-微信登录 phone-手机号注册登录 |

### 13.4 积分规则一览

| 行为 | 变动类型 | 积分变动 | 限制 | 当前状态 |
|------|----------|----------|------|----------|
| 每日签到 | sign_in | +5 | 每天一次 | ✅ 生效中 |
| 首次完善宠物信息 | pet_complete | +20 | 每只宠物仅首次 | ✅ 生效中 |
| 兑换商品 | exchange | -对应积分 | 需 ≥ 商品积分价格 | ✅ 生效中 |
| 管理员操作 | admin | ±N/A | 无限制 | ✅ 后台可用 |
| 发布动态（审核通过） | post | +10 | 每条动态仅发放一次 | ⛔ 已下架 |
| 动态被点赞 | like | +1 | 每日上限20分 | ⛔ 已下架 |
| 动态被评论 | comment | +2 | 每日上限20分 | ⛔ 已下架 |

> **合规说明**：因前端已移除发布动态、点赞、评论入口，上述三项社交类积分**不再产生**。
> 当前有效获取途径为「每日签到」与「首次完善宠物信息」。
> 若后续需要为「记录体重」「添加成长相册」发放积分，需在 `PetHealthServiceImpl` 中接入
> `IUserPointsService` 并补充去重校验（同一宠物同一记录行为仅发放一次）。
...
| 实体 | 字段 | 格式说明 |
|------|------|----------|
| PetAlbum | images | JSON数组字符串，如 `["/profile/upload/a.jpg"]`；仅本人可见 |
| PetPost（已下架） | images | JSON数组字符串，如 `["url1","url2"]` |
| ShopProduct | productImages | JSON数组字符串，如 `["url1","url2"]` |

### 13.6 登录方式返回 Token 对比
...
| 无权操作他人的宠物数据 | 读写非本人宠物的体重记录 | POST /pet/weight、GET /pet/{petId}/weight |
| 无权查看他人的相册记录 | 查看非本人成长相册 | GET /pet/album/{id} |
| 无权删除他人的记录 | 删除非本人体重记录 | DELETE /pet/weight/{id} |
| 相册记录不存在 | 记录ID无效或已删除 | GET/PUT/DELETE /pet/album/{id} |
| 文章不存在或已下架 | 文章未发布或已删除 | GET /articles/{id} |
| 请先选择宠物 | 缺少 petId 参数 | POST /pet/weight、POST /pet/album |
